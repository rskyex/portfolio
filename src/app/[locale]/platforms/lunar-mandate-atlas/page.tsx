...
import { getLunarMandateAtlasContent } from '@/content/platforms-lunar-mandate-atlas';
...
export async function generateMetadata({
  ...
  const c = getLunarMandateAtlasContent(locale as Locale);
  return {
    title: c.meta.title,
    description: c.meta.description,
    alternates: buildAlternates('/platforms/lunar-mandate-atlas'),
    openGraph: {
      title: c.meta.title,
      description: c.meta.description,
      images: [
...
export default async function LunarMandateAtlasPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 md:py-20">
      ...
      <div className="mt-8 space-y-8">
        ...
        <div className="panel rounded-sm p-8">
          <h3 className="font-noto-sans text-base font-medium text-shiro/90 mb-4">{c.keyDimensions.heading}</h3>
          <ul className="space-y-2">
            {c.keyDimensions.items.map((item, i) => (
