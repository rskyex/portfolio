...
import { getSecondSelfContent } from '@/content/platforms-second-self';
...
export async function generateMetadata({
  ...
  const c = getSecondSelfContent(locale as Locale);
  return {
    title: c.meta.title,
    description: c.meta.description,
    alternates: buildAlternates('/platforms/second-self'),
    openGraph: {
      title: c.meta.title,
      description: c.meta.description,
...
export default async function SecondSelfPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 md:py-20">
      ...
      <div className="mt-8 space-y-8">
        ...
        <div className="panel rounded-sm p-8">
          <h3 className="font-noto-sans text-base font-medium text-shiro/90 mb-4">{c.cardSystem.heading}</h3>
          <div className="space-y-4">
            {c.cardSystem.categories.map(category => (
              <div key={category.title}>
                <h4 className="font-noto-sans text-sm font-medium text-kin/70 mb-1">{category.title}</h4>
        ...
        <div className="panel rounded-sm p-8">
          <h3 className="font-noto-sans text-base font-medium text-shiro/90 mb-4">{c.victory.heading}</h3>
          <p className="font-noto-sans text-sm text-shiro/90 leading-relaxed mb-4">
            {c.victory.paragraphs[0]}
          </p>
          <p className="font-noto-sans text-sm text-shiro/90 leading-relaxed">
            {c.victory.paragraphs[1]}
          </p>
        </div>
