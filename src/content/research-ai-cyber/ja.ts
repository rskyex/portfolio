...
export const aiCyberJa: AICyberContent = {
  ...
  overview: {
    ...
    paragraphs: [
      ...
      '「責任ある行動」が、宇宙・AI・サイバーの各ガバナンス領域において、いかにして未完の規範として機能するのかを検討する。これを確立された概念として扱うのではなく、運用上の枠組みへの成文化を妨げているガバナンスの空隙、曖昧性、戦略的な力学を考察する。',
      // TODO(i18n-review): denial=否認 / displacement=転位 / inoculation=予防接種 の訳語要確認。デュアルユースの曖昧性（glossary: review）要確認
      'AIガバナンスの言説に働く三つの力学、すなわち否認・転位・予防接種をたどり、国家や諸機関が、実質的なガバナンス上のコミットメントを構造的に先送りしながら、いかにして責任の言説を持ち出すのかを探究する。こうしたパターンを、デュアルユースの曖昧性、規範の成文化、先端技術ガバナンスのアーキテクチャをめぐるより広範な問いへと結びつける。',
    ],
  },
