...
export const faultlineJa: FaultlineContent = {
  ...
  overview: {
    ...
    paragraphs: [
      ...
      'Faultline は、4つの領域特化型ツール——Orbital Risk Tracker、Global Nuclear Infrastructure Atlas、Cyber Escalation Atlas、Lunar Mandate Atlas——を単一の分析スイートへと統合した、統一的な戦略インフラ・リスク・プラットフォームである。',
      // TODO(i18n-review): legitimacy（正統性）/ authority（権威）/ escalation の訳語要確認
      '各モジュールは、軌道・核・サイバー・月面の各領域にわたってガバナンス理論を実装可能なプラットフォームへと翻訳し、正統性・権威・エスカレーションに関する概念的枠組みを、応用的な政策上の問いと結びつける。',
    ],
  },
  ...
  subProjects: [
    {
      ...
      description:
        ...
        '対衛星事象、近接運用、GNSSジャミング事案、サイバー・宇宙の連関を追跡する。ジャービスの安全保障のジレンマの枠組みと武器化された相互依存の理論に依拠し、軌道インフラ全体にわたるエスカレーション分析を支援する。',
    ...
    },
    {
      ...
      description:
        ...
        '世界の民生用原子力発電インフラ、主要な事故サイト、推定核戦力、ならびに関連する放射線・健康の文脈をインタラクティブにマッピングする。教育および政策の場での利用を想定し、IAEA、SIPRI、WHO などのデータに基づいている。',
    ...
    },
    {
      ...
      description:
        ...
        'サイバー事案を、戦略的行動とガバナンスのシグナルの表れとしてマッピングする。ケロの「unpeace（不平和）」概念、アトリビューションの枠組み、シェリングのエスカレーション論理、そしてタリン・マニュアル2.0を取り扱い、サイバー作戦をより広い戦略分析のなかで文脈づける。',
    ...
    },
    {
      ...
      description:
        ...
        '競合する月面ガバナンスの枠組み、マンデート、権威アーキテクチャをインタラクティブにマッピングする。アルテミス合意と国際月面研究ステーション（ILRS）計画を比較し、普遍主義的な主張がいかに正統化の装置として機能し、ガバナンス・アーキテクチャが月面で許容される行動をいかに形づくるかを跡づける。',
