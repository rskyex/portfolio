...
export const lunarJa: LunarContent = {
  ...
  abstract: {
    ...
    paragraphs: [
      ...
      '本稿は、競合する月面ガバナンスの枠組みが、いかにして自らの権威を構築し、人類を代表して正統性を主張するのかを検討する。アルテミス合意と国際月面研究ステーション（ILRS）計画の比較分析を通じて、それぞれの枠組みを支える正統化の受け手、翻訳装置、そして権威アーキテクチャを考察する。',
      // TODO(i18n-review): 普遍主義的言説/正統化装置/境界形成（glossary: review）要確認
      'いずれの枠組みも普遍主義的な言説を用い——「人類のために」「全人類のために」あるいは「平和的目的のために」と語りながら——相異なる参加条件、ガバナンス規範、境界形成の実践を確立している。本稿は、こうした普遍主義的な主張が、包摂と排除を同時に行う正統化の装置として機能し、月面ガバナンスにおける正統な権威の構築が、確立された制度的達成ではなく、継続的かつ争われるプロセスであることを論じる。',
      // TODO(i18n-review): 正統化理論（glossary: review）要確認
      '24点の文書からなるコード化されたコーパスと、正統化理論に由来する概念的ツールに依拠し、本研究は、各枠組みがいかにして広範な原則を具体的なガバナンス・アーキテクチャへと翻訳していくのか——そしてこれらのアーキテクチャが、ひいては月面で許容される行為の境界をいかに形づくるのか——をたどる。',
    ],
  },
  ...
  methodology: {
    ...
    steps: [
      ...
      { glyph: '二', text: '正統化のマーカー、権威の主張、境界形成の言説に焦点を当てた独自のコードブック設計' },
  ...
  keyThemes: {
    ...
    items: [
      ...
      '普遍主義的言説が、競合するガバナンスの枠組みにおいて正統化の装置としていかに機能するか',
