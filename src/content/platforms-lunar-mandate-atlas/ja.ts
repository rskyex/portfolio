...
export const lunarMandateAtlasJa: LunarMandateAtlasContent = {
  ...
  overview: {
    ...
    paragraphs: [
      ...
      'Lunar Mandate Atlas は、月において立ち現れつつある秩序を形づくる、競合するガバナンスの枠組み・マンデート・権威アーキテクチャを地図化するインタラクティブなプラットフォームである。アルテミス合意と国際月面研究ステーション（ILRS）計画を比較し、それぞれの枠組みがいかに自らの正統性を構築し、人類を代表して行動すると主張するのかを追跡する。',
      ...
      'いずれの枠組みも、「人類のために」「全人類のために」あるいは「平和的目的のために」と普遍主義的な言説を掲げながら、その実、相異なる参加条件・ガバナンス規範・境界形成（バウンダリー・メイキング）の実践を打ち立てている。本プラットフォームは、こうした普遍主義的な主張が、包摂と排除を同時にもたらす正統化の装置としていかに機能するかを可視化する。',
      // TODO(i18n-review): legitimation theory（正統化理論, review）/ governance architectures / permissible action 要確認
      '本アトラスは正統化理論に依拠し、広範な原則がいかに具体的なガバナンスのアーキテクチャへと翻訳されるか、そしてそのアーキテクチャがひるがえって月における許容される行動の境界をいかに形づくるかを追跡する。',
    ],
  },
  keyDimensions: {
    heading: '主要な次元',
